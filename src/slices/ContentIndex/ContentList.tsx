      ...
      <ul className='grid border-b border-b-slate-100 '>
        ...
        {items.map((item, index)=>(
          ...
          {isFilled.keyText(item.data.title) && (
            <li className='list-item opacity-0 ' key = {index} onMouseEnter = {() => onMouseEnter(index)} onMouseLeave={onMouseLeave} ref = {(el)=>(itemsRef.current[index] = el)}>
              
            <Link href={urlPrefixes + "/" + item.uid} className='justify-between flex flex-col  border-t border-t-slate-100 py-10 text-slate-200 md:flex-row'
